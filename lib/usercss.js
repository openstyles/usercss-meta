...
/* global URL */
/* eslint-disable no-div-regex, no-multi-assign, no-warning-comments, no-prototype-builtins, quotes, quote-props, unicorn/catch-error-name, prefer-promise-reject-errors */

const stylus = require('accord').load('stylus');
const semverCompare = require('semver').compare;
const colorConverter = require('./colorconverter');
const styleCodeEmpty = require('./utils');
  ...
  function buildCode(style) {
    ...
    if (preprocessor) {
      if (!BUILDER[preprocessor]) {
        return Promise.reject(`Unsupported @preprocessor: ${preprocessor}`);
      }
    ...
    return (
      Promise.resolve(
        ...
        sourceCode)
      /* We're only parsing the metadata

...
function validateRange(state) {
  ...
  if (result.step != null) {
    for (const num of [value, result.min, result.max]) {
      if (num != null && !isMultipleOf(num, result.step)) {
        throw new ParseError({
...
function isMultipleOf(value, step) {
  ...
  const scale = decimal ? 10 ** decimal.length : 1;
  return value * scale % (step * scale) === 0;
}

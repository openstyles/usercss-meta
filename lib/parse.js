...
/* Inlining semver-regex v2.0.0 here instead of unreliably pinning it in package.json.
 * This is the last version without look-behind so it can run in older browsers supported by Stylus.
 * https://github.com/sindresorhus/semver-regex/releases/tag/v2.0.0 */
// eslint-disable-next-line unicorn/better-regex
const RE_SEMVER = /\bv?(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)(?:-[\da-z-]+(?:\.[\da-z-]+)*)?(?:\+[\da-z-]+(?:\.[\da-z-]+)*)?\b/ig;
...
function validateVersion(state) {
  const {value} = state;
  const match = value.match(RE_SEMVER);
  // The second check for the entire value is necessary with semver-regex v2.0.0
  ...
  state.value = normalizeVersion(value);
}
...
function isMultipleOf(value, step) {
  ...
  return Math.abs(n - nInt) < 10 ** (`${nInt}`.length - 16);
}
...
function createParser({
  ...
  const parser = {__proto__: null, ...DEFAULT_PARSER, ...userParseKey};
  const varParser = {...DEFAULT_VAR_PARSER, ...userParseVar};
  const validator = {...DEFAULT_VALIDATOR, ...userValidateKey};
  const varValidator = {...DEFAULT_VAR_VALIDATOR, ...userValidateVar};

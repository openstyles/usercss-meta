...
function LevenshteinDistanceWithMax(firstString, secondString, maxEdit) {
  ...
  for (let i = 1; i <= lenTwo; i++) {
    ...
    let miniumCostRow = maxEdit + 1;
    for (let j = 1; j <= lenOne; j++) {
      oldDistanceCost = distance[j];
      let miniumCost = distance[j] + 1;
      if ((distance[j - 1] + 1) < miniumCost) {
        miniumCost = distance[j - 1] + 1;
      }

      editCost = firstString[j - 1] === secondString[i - 1] ? 0 : 1;
      if ((lastDistanceCost + editCost) < miniumCost) {
        miniumCost = lastDistanceCost + editCost;
      }

      if (miniumCost < miniumCostRow) {
        miniumCostRow = miniumCost;
      }

      distance[j] = miniumCost;
      lastDistanceCost = oldDistanceCost;
    }

    // Check after each row if maxEdit is less than miniumCostRow
    // to know when to stop early on.
    if (miniumCostRow > maxEdit) {
      return false;
    }
  }
